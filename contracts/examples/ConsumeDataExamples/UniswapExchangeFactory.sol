...
contract ExchangeFactory is Ownable {
    ...
    function change_factory(address _uniswapfactory)
        public
        onlyOwner
        returns (bool)
    {
    ...
    function getExchangefromToken(address _erctoken)
        public
        view
        returns (address)
    {
        address _exchange;
        UniswapFactoryInterface unifactory = UniswapFactoryInterface(
            uniswapfactory
        );
    ...
    function addliquidity(uint256 tokens) public payable {
        ...
        UniswapExchangeInterface exchangeinter = UniswapExchangeInterface(
            myexchange
        );
        exchangeinter.addLiquidity.value(msg.value)(
            0,
            tokens,
